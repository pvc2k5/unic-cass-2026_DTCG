...
module tb_top_FLL;
    ...
    initial begin
        ui_PAD2CORE[0] = 1'b0; 
        forever #50 ui_PAD2CORE[0] = ~ui_PAD2CORE[0];//10MHz
    end
    ...
    task measure_clock_period;
	 ...
	 while(count < N) begin
            @(uo_CORE2PAD[1]);
            if(uo_CORE2PAD[1] === 1'b1)
                count = count + 1;
        end
       	 t2 = $realtime;
       	 period = (t2 - t1) / N;
       	 $display("[%t] Period = %0.3f ns | Freq = %0.3f MHz",
                  $time, period, 1000.0/period);
    	end
    endtask
    integer j;
    initial begin 
	...
	ui_PAD2CORE[3] = 0;
        ...
        #500 rst_ni = 1;
	#1000;
	spi_write(8'h00, 32'h00000003);  // DCO mode
	spi_write(8'h01, 32'h04000000);
	#50000;
	spi_write(8'h00, 32'h0000003D);  // FLL mode, div=15
	#200000;
	$display("write complete");
        $display("-------------------------------------------------------------");
	$display("Time          | Measured Output Clock");
